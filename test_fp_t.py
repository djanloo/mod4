from mod4.diffeq import time_dependent_funker_plank
import numpy as np
import matplotlib
...
for gamma in [2.1]:
    ...
    f_t = 0.01*np.sin(np.arange(integration_args['n_steps'])*integration_args['dt'])

    period = np.pi*2*np.sqrt(1.0/integration_args['alpha'])
    print(f"period is roughly {period}")

    p , norm = time_dependent_funker_plank(p0, x, v, f_t, integration_args)
    p = np.array(p)
