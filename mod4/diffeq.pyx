...
cpdef preburgers(double [:] u0, double v=0.5, double dx=0.1, double dt=0.1, int n_steps = 10):
  """Solves Burgers' equation with zero diffusion:

  du/dt + u du/dx = 0

  Uses a Lax-Wendroff startegy.
  ...
  for _ in range(n_steps):
    ...
    for i in range(N):
      left = 0.0 if i == 0 else u[i-1]
      right = 0.0 if i == N-1 else u[i+1]
      # instead of v, the propagation velocity is the mean function value
      u_intermediate[i] = 0.5*(right+left) - (right+left) * dt/dx/2 * (right - left) 
  
    # LEAPFROG STEP
    for i in range(N):
      left = 0.0 if i == 0 else u_intermediate[i-1]
      right = 0.0 if i == N-1 else u_intermediate[i+1]
      u_next_time[i] = u[i] - (right+left) * dt/dx * (right - left) 
