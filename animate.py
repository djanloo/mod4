...
mu_n_plot, = axes['mean'].plot([0, 1], [0, 0])
mu_an_plot, = axes['mean'].plot([0,1],[0,0])
sigma_an_plot = axes['var'].plot([0,1],[0,0])
sigma_num_plot = axes['var'].plot([0,1],[0,0])
...
def update(i):
    ...
    axes['mean'].clear()
    axes['mean'].plot(np.linspace(t0, t, len(mu_num)),mu_num)
    axes['mean'].plot(np.linspace(t0, t, len(mu_an)), mu_an)
    axes['mean'].set_xlim(0, FRAMES*integration_params['dt']*integration_params['n_steps'])
    axes['mean'].set_ylim(-2,2)

    axes['var'].clear()
    axes['var'].plot(np.linspace(t0, t, len(sigma_num)),sigma_num)
    axes['var'].plot(np.linspace(t0, t, len(sigma_an)), sigma_an)
