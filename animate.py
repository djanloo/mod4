...
from mod4.diffeq import funker_plank, time_dependent_funker_plank
...
def update(i):
    ...
    time = np.arange(i, i + integration_args['n_steps'])*integration_args['dt']
    print("lentime", len(time))
    print("i", i)

    f_t = 0.001*np.sin(time)

    p , norm = time_dependent_funker_plank(p, x, v, f_t, integration_args)
